#!/usr/bin/env python3
"""Train a routed TeamLoRA candidate-wise POI reranker with RAAT + FSDP.
...
This variant keeps the model architecture unchanged and only adapts the
training runtime for torchrun/FSDP so parameters, gradients, and optimizer
states can be sharded across multiple GPUs.
"""
...
def setup_distributed(args: argparse.Namespace) -> tuple[torch.device, bool, int, int, int]:
    if not distributed_enabled():
        device = torch.device(args.device if torch.cuda.is_available() or args.device == "cpu" else "cpu")
        return device, False, 0, 0, 1
    if not torch.cuda.is_available():
        raise RuntimeError("FSDP distributed training requires CUDA.")
    local_rank = int(os.environ["LOCAL_RANK"])
    rank = int(os.environ["RANK"])
    world_size = int(os.environ["WORLD_SIZE"])
    torch.cuda.set_device(local_rank)
    dist.init_process_group(backend="nccl", device_id=torch.device("cuda", local_rank))
    return torch.device("cuda", local_rank), True, rank, local_rank, world_size
...
def rank0_print(rank: int, *values: Any, **kwargs: Any) -> None:
    if is_rank0(rank):
        print(*values, **kwargs)
...
def model_uses_graph_prior(model: nn.Module) -> bool:
    return bool(getattr(unwrap_model(model), "use_graph_prior", False))
...
def model_group_loss(
    ...
    output = model(text_batch, graph_features_tensor, return_parts=bool(model_uses_graph_prior(model) and residual_l2 > 0))
...
def evaluate(model: TeamLoRAReranker, tokenizer: Any, loader: DataLoader, args: argparse.Namespace, device: torch.device) -> Dict[str, float]:
    ...
    module = unwrap_model(model)
    if getattr(module, "use_graph_prior", False):
        metrics["residual_alpha"] = float(F.softplus(module.residual_alpha_param).detach().cpu().item())
        metrics["graph_prior_type"] = str(module.graph_prior_type)
        metrics["residual_bound_mode"] = str(module.residual_bound_mode)
        metrics["residual_bound_value"] = float(module.residual_bound_value)
    return metrics
...
def main() -> None:
    ...
    args = parse_args()
    device, distributed, rank, local_rank, world_size = setup_distributed(args)
    set_seed(args.seed)
    if is_rank0(rank):
    ...
    rank0_print(rank, json.dumps(data_summary, ensure_ascii=False, indent=2))
    ...
    if distributed:
        ...
        mp_policy = MixedPrecision(
            param_dtype=torch.bfloat16 if args.bf16 else torch.float16 if args.fp16 else torch.float32,
            reduce_dtype=torch.bfloat16 if args.bf16 else torch.float16 if args.fp16 else torch.float32,
            buffer_dtype=torch.bfloat16 if args.bf16 else torch.float16 if args.fp16 else torch.float32,
        )
        model = FSDP(
            model,
    ...
    rank0_print(rank, json.dumps({"lora_replaced_modules": replaced, "trainable_params": trainable, "total_params": total}, ensure_ascii=False))
    rank0_print(
        rank,
        json.dumps(
            {
                ...
                "model_graph_prior": bool(unwrap_model(model).use_graph_prior),
                "model_graph_prior_type": str(unwrap_model(model).graph_prior_type),
                "model_residual_bound_mode": str(unwrap_model(model).residual_bound_mode),
                "model_residual_bound_value": float(unwrap_model(model).residual_bound_value),
                "distributed": bool(distributed),
                "world_size": int(world_size),
                "fsdp": bool(distributed),
                "fsdp_sharding_strategy": "FULL_SHARD" if distributed else "none",
            },
            ensure_ascii=False,
        )
    )
    ...
    if explicit_eval_steps:
        ...
        rank0_print(rank, json.dumps({"eval_at_steps": explicit_eval_steps, "eval_mode": "explicit"}, ensure_ascii=False))
    ...
    progress = tqdm(total=total_steps, desc="train", disable=not is_rank0(rank))
    while global_step < total_steps:
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if global_step % args.logging_steps == 0:
                ...
                rank0_print(rank, json.dumps({"step": global_step, "loss": avg_loss, "lr": scheduler.get_last_lr()[0]}, ensure_ascii=False))
            if explicit_eval_steps:
            ...
            if should_eval:
                save_trainable_checkpoint(
                    ...
                    metadata | {"step": global_step, "checkpoint_type": "latest_before_eval"},
                    distributed,
                    rank,
                )
                metrics = evaluate(model, tokenizer, val_loader, args, device)
                metrics["step"] = global_step
                rank0_print(rank, json.dumps(metrics, ensure_ascii=False))
                if is_rank0(rank):
                ...
                if metrics["mrr"] > best_mrr:
                    ...
                    save_trainable_checkpoint(
                        ...
                        metadata | {"best_step": global_step, "best_metrics": metrics},
                        distributed,
                        rank,
                    )
                model.train()
            if global_step % args.save_steps == 0 and not should_eval:
                save_trainable_checkpoint(
                    model,
                    args.output_dir / "latest",
                    metadata | {"step": global_step, "checkpoint_type": "latest"},
                    distributed,
                    rank,
                )
    ...
    save_trainable_checkpoint(
        model,
        args.output_dir / "latest",
        metadata | {"final_step": global_step, "checkpoint_type": "latest_final"},
        distributed,
        rank,
    )
