#!/usr/bin/env python3
"""LoRA/QLoRA SFT for LoRA-C candidate booster.

This trainer is intentionally separate from the 8B POI predictor trainer.
It trains a 1B model to output only:
    {"supplemental_poi_ids":["v..."]}
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train LoRA-C candidate booster.")
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/candidate-booster-lora-llama32-1b-missed-only"))
    p.add_argument("--init-lora-path", type=Path, default=None)
    p.add_argument("--max-source-length", type=int, default=1536)
    p.add_argument("--max-target-length", type=int, default=96)
...
@dataclass
class CandidateBoosterDataset(Dataset):
    rows: List[Dict[str, Any]]
    tokenizer: Any
    ...
    def __getitem__(self, idx: int) -> Dict[str, List[int]]:
        ...
        if "supplemental_poi_ids" not in target:
            raise ValueError(f"{row.get('sample_id')} target must contain supplemental_poi_ids")
...
def main() -> None:
    ...
    train_ds = CandidateBoosterDataset(train_rows, tokenizer, args.max_source_length, args.max_target_length)
    val_ds = CandidateBoosterDataset(val_rows, tokenizer, args.max_source_length, args.max_target_length)
