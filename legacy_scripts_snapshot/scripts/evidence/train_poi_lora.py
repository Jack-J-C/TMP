#!/usr/bin/env python3
"""LoRA/QLoRA SFT for the 8B next-POI predictor.

The same trainer is used by Stage1 RAW warmup, Stage2 mixed training, and
Stage3 hard-focused finetuning.
"""
...
def patch_torch_set_submodule() -> None:
    ...
    Some torch builds expose get_submodule but not set_submodule, while recent
    transformers/bitsandbytes 4-bit loading expects set_submodule to exist.
    """
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train next-POI predictor with LoRA/QLoRA.")
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/poi-lora-llama31-8b"))
    p.add_argument(
        "--init-lora-path",
        type=Path,
        default=None,
        help="Optional existing LoRA adapter to continue training from, e.g. Stage1 -> Stage2.",
    )
    ...
    p.add_argument("--load-in-4bit", action="store_true", help="Use QLoRA 4-bit loading.")
...
class POISFTDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, List[int]]:
        ...
        target = str(messages[2].get("content") or "").strip() + self.tokenizer.eos_token
        source = self.tokenizer.apply_chat_template(source_messages, tokenize=False, add_generation_prompt=True)
...
def main() -> None:
    ...
    train_ds = POISFTDataset(train_rows, tokenizer, args.max_source_length, args.max_target_length)
    val_ds = POISFTDataset(val_rows, tokenizer, args.max_source_length, args.max_target_length)
